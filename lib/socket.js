  ...
  io.on("connection", (socket) => {
    socket.on("signin", (userNick) => {
      ...
      socket.on("join", async (room) => {
        ...
        console.log(`@@ 게임방 입장 (${userNick}) to (${room})`);
        socket.join(room);

        const message = `${userNick}님이 입장하셨습니다.`;
        io.to(room).emit("adminMessage", { message });
        ...
        socket.on("sendMessage", async (message, sender) => {
          logger.info(`@@ 메시지 전송 (${sender}): ${message} (${room})`);
          ...
          io.emit("message", { sender, message, room });
        });

        socket.on("leave", (room) => {
          logger.info(`@@ 게임방 퇴장 (${userNick}) from (${room})`);
          socket.leave(room);

          const message = `${userNick}님이 퇴장하셨습니다.`;
    ...
    socket.on("disconnect", ({ username }) => {
      logger.info(`@@ (-) 사용자 연결 해제: ${username} (${socket.id})`);
    });
  });
};
