...
export default function GameEasy() {
  ...
  // 이미지는 27개인데 필요한 카드는 12개로 고정되어 있기 때문에 27개의 이미지 중 랜덤으로 12개를 뽑도록 구현
    ...
    // 랜덤 값 뽑기
      const randomNumber = getRandom(27, 0);
  ...
  function showCardDeck() {
    ...
    const showCardPromise = new Promise((resolve, reject) => {
  ...
  gameBoard.addEventListener('click', (e) => {
    ...
    if (e.target.parentNode.className === 'card') {
      const clickCardId = e.target.parentNode.dataset.id;
  ...
  // 화면에서 앞면으로 보이도록 스타일 조정
    ...
    const openCardIndexArr = getOpenCardArr(id);
  ...
  function checkCardMatch(indexArr) {
    const firstCard = cardDeck[indexArr[0]];
    const secondCard = cardDeck[indexArr[1]];
